# MLP Test Accuracy: 73.08%
#    3.672297 seconds (8.80 M allocations: 1.571 GiB, 12.94% gc time, 44.27% compilation time)
...
function build_mlp()
    return Chain(
        Dense(228, 128, relu),   # Hidden Layer 1
        Dropout(0.2),            # Prevent overfitting on small data
        Dense(128, 64, relu),    # Hidden Layer 2
        Dense(64, 2)             # Output Layer (2 classes)
        )
end
